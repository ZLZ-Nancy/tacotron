...
from datasets import blizzard, ljspeech, beijing, thchs
...
def process_beijing(args):
  in_dir = os.path.join(args.base_dir, 'Beijing')
  out_dir = os.path.join(args.base_dir, args.output)
  os.makedirs(out_dir, exist_ok=True)
  metadata = beijing.build_from_path(in_dir, out_dir, args.num_workers, tqdm=tqdm)
  write_metadata(metadata, out_dir)
...
def preprocess_thchs(args):
  ...
  out_dir = os.path.join(args.base_dir, args.output)
  os.makedirs(out_dir, exist_ok=True)
  metadata = thchs.build_from_path(in_dir, out_dir, args.num_workers, tqdm=tqdm)
  write_metadata(metadata, out_dir)
...
def main():
  ...
  parser.add_argument('--dataset', required=True, choices=['blizzard', 'ljspeech', 'beijing', 'thchs-30'])
